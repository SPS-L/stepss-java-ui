    ...
    private static void checkExtraDirectorySearch() {
        ...
        try {
            ...
            expect("a stub compiler that is not on PATH is not found by the PATH-only lookup",
                    null, my.stepss.platform.PlatformLauncher.findOnPath(stub));

            java.io.File hit = my.stepss.platform.PlatformLauncher.findOnPath(stub,
            ...
            expect("a name in neither PATH nor the extra directories is still not found",
                    null, my.stepss.platform.PlatformLauncher.findOnPath(
    ...
    private static void checkGuardFailureClearsBuiltExecutable() {
        ...
        try {
            ...
            ModelCompiler compiler = new ModelCompiler(
                    my.stepss.platform.Platform.current(),
                    new my.stepss.platform.Toolchain(
                            my.stepss.platform.Platform.current(), toolDir));
